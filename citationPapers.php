...
<div id="content">
	...
	<div id="main">
	    <div id="area">
		    <h3>Citation Papers</h3>
			<?php 
				$nextyear=$currentyear + 1;
				$nextyear_newyearsdate=$nextyear."-01-01";
				$sqlC = "select count(p_id) as count from paper where p_citation is not null ";
				$sql1 = "select * from paper where p_citation is not null order by p_date desc limit 0, $per_citation";
			?>
			<?php
			...
			<div class="item">
                ...
                <?php }?>
				<div class="rich_text"><?php echo $row1[p_citation]?></div>
			</div>
			<?php
			    }
				$sql = "select count(p_id) as count from paper where p_citation is not null ";
			    $result = mysql_query($sql);
			    if ($row = mysql_fetch_array($result)) {
    			    if ($row["count"] > $per_citation) {
			?>
		        <div id="more" class="more"><a href="javascript:more_citation(<?php echo $index.",".$currentDisplayYear;?>)">more<em></em></a></div>
		    <?php
		            } else {
		    ?>
		    <br/>
